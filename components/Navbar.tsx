...
import { NAV_LINKS, SparkIcon, APP_ABBREV } from '../constants';
...
const SparkLogo: React.FC<{ onNavigate: (page: string) => void }> = ({ onNavigate }) => (
  <button onClick={() => onNavigate('home')} className="flex items-center space-x-2 focus:outline-none group">
    ...
    <span className="text-2xl font-black text-[var(--text-primary)] tracking-tighter">{APP_ABBREV}</span>
  </button>
);
...
const Navbar: React.FC<{ 
  ...
  currentPage?: string
}> = ({ onNavigate, user, onLogout, isDarkMode, toggleTheme, themeMode, currentPage }) => {
  ...
  const dashboardPage = user?.role === 'Admin' ? 'admin-dashboard' :
    user?.role === 'Brand' ? 'brand-dashboard' :
      (user?.role === 'Organization' || user?.role === 'Association') ? 'association-dashboard' : 'creator-dashboard';
        ...
        <div className="flex items-center justify-between h-20 md:h-24">
          <div className="flex items-center">
            <button onClick={() => onNavigate('home')} className="flex items-center space-x-2 focus:outline-none group">
              ...
              <span className="text-lg md:text-xl font-fancy font-black text-[var(--text-primary)] tracking-tighter">{APP_ABBREV}</span>
            </button>
          </div>
          ...
          <div className="hidden lg:block">
            <div className="ml-10 flex items-baseline space-x-1">
              {NAV_LINKS.map((link: NavLink) => {
                const isActive = currentPage === link.href || (link.href === 'home' && currentPage === 'home');
                return (
                  <a
                    key={link.label}
                    href={link.href}
                    ...
                    className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${
                      isActive
                        ? 'text-spark-red bg-spark-red/10 font-black'
                        : 'text-[var(--text-secondary)] hover:text-spark-red hover:bg-spark-red/5'
                    }`}
                  >
                    {link.label}
                    {isActive && <span className="block h-0.5 w-full bg-spark-red mt-0.5 rounded-full" />}
                  </a>
                );
              })}
            </div>
            ...
            {user ? (
              <>
                <button
                  ...
                  className="text-[var(--text-primary)] hover:text-spark-red font-black text-sm uppercase tracking-widest px-4 py-2 transition-colors flex items-center gap-2"
                >
                ...
                <button
                  ...
                  className="glass text-[var(--text-primary)] font-bold py-3 px-6 rounded-2xl transition-all active:scale-95 text-sm border border-[var(--border-color)]"
                >
                  Log Out
                </button>
              </>
              ...
              <>
                <button
                  ...
                  className="text-[var(--text-secondary)] hover:text-spark-red font-bold text-sm uppercase tracking-widest px-4 py-2 transition-colors"
                >
                  Login
                </button>
                <button
                  onClick={() => onNavigate('create-account')}
                  className="bg-spark-red hover:bg-red-700 text-white font-bold py-4 px-8 rounded-2xl transition-all shadow-lg shadow-spark-red/20 active:scale-95 text-sm"
                >
                  Get Started
                </button>
              </>
            ...
            {NAV_LINKS.map((link: NavLink) => {
              ...
              return (
                <a
                  ...
                  className={`block px-4 py-4 rounded-2xl text-base font-bold transition-colors flex items-center justify-between ${
                    isActive
                      ? 'text-spark-red bg-spark-red/10'
                      : 'text-[var(--text-secondary)] hover:text-spark-red hover:bg-spark-red/5'
                  }`}
                >
                  {link.label}
                  {isActive && <span className="w-2 h-2 rounded-full bg-spark-red" />}
                </a>
              );
            })}
            <div className="pt-6 border-t border-[var(--border-color)] flex flex-col space-y-4">
              {user ? (
                <>
                  ...
                  <button onClick={handleLogout} className="w-full text-center bg-[var(--text-primary)] text-[var(--bg-primary)] font-black py-4 rounded-2xl uppercase tracking-widest text-sm hover:bg-spark-red hover:text-white transition-all shadow-lg shadow-black/5">
                    Log Out
                  </button>
                </>
              ) : (
                <>
                  <button onClick={() => { onNavigate('login'); setIsOpen(false); }} className="w-full text-center bg-[var(--text-primary)] text-[var(--bg-primary)] font-black py-4 rounded-2xl uppercase tracking-widest text-sm hover:bg-spark-red hover:text-white transition-all shadow-lg shadow-black/5">
                    Login
                  </button>
